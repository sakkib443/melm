...
import { FiSave, FiX, FiCamera, FiArrowLeft, FiPlus, FiTag, FiDollarSign, FiLayers } from "react-icons/fi";
...
export default function CreatePhotoPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        type: "landscape", category: "", tags: [], thumbnail: "",
        images: 0, resolution: "", format: "JPG",
    ...
    useEffect(() => {
        if (isEditMode && editId) {
            const fetchData = async () => {
                setLoading(true);
                try {
                    const response = await photoService.getById(editId);
                    if (response.success && response.data) {
                        ...
                        setFormData({
                            ...
                            images: data.images || 0, resolution: data.resolution || "",
    ...
    const handleSubmit = async (e) => {
        ...
        setSaving(true);
        try {
            const payload = { ...formData, price: Number(formData.price), salePrice: formData.salePrice ? Number(formData.salePrice) : null, images: Number(formData.images) };
            if (isEditMode) { await photoService.update(editId, payload); toast.success("Updated!"); }
            else { await photoService.create(payload); toast.success("Created!"); }
            router.push("/dashboard/admin/products/photos");
        ...
        <div className="p-6 lg:p-8">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/admin/products/photos" className="w-10 h-10 rounded-md border border-gray-200 dark:border-gray-700 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800"><FiArrowLeft /></Link>
                    <div><h1 className="text-lg font-semibold text-gray-900 dark:text-white">{isEditMode ? "Edit Photo Pack" : "Create Photo Pack"}</h1><p className="text-xs text-gray-500">{isEditMode ? "Update photo pack details" : "Add new photo pack"}</p></div>
                </div>
                <div className="flex items-center gap-2">
                    <Link href="/dashboard/admin/products/photos" className="btn btn-ghost"><FiX size={16} /> Cancel</Link>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-6">
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><FiCamera size={16} /> Basic Information</h2>
                    ...
                    <div className="space-y-6">
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            ...
                            <div className="space-y-4">
                                ...
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Photo Type</label><select name="type" value={formData.type} onChange={handleChange} className="input w-full">{PHOTO_TYPES.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select></div>
                                ...
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Number of Photos</label><input type="number" name="images" value={formData.images} onChange={handleChange} className="input w-full" min="0" /></div>
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Resolution</label><input type="text" name="resolution" value={formData.resolution} onChange={handleChange} className="input w-full" placeholder="e.g. 4K, 6000x4000" /></div>
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Format</label><input type="text" name="format" value={formData.format} onChange={handleChange} className="input w-full" placeholder="e.g. JPG, PNG, RAW" /></div>
