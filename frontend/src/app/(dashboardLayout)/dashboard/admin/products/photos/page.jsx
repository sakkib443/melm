...
import { FiPlus, FiSearch, FiCamera, FiLoader, FiEdit, FiTrash2, FiEye, FiRefreshCw, FiDownload, FiStar } from "react-icons/fi";
...
export default function PhotosPage() {
    ...
    const mockData = [
        { _id: "1", title: "Nature Landscape Pack", thumbnail: "https://via.placeholder.com/400x300/6366f1/fff?text=Nature", price: 999, salePrice: 499, type: "landscape", status: "published", downloads: 1234, rating: 4.8, images: 50 },
        { _id: "2", title: "Business Portrait Collection", thumbnail: "https://via.placeholder.com/400x300/ec4899/fff?text=Portrait", price: 1499, salePrice: null, type: "portrait", status: "published", downloads: 567, rating: 4.9, images: 30 },
        { _id: "3", title: "Abstract Backgrounds", thumbnail: "https://via.placeholder.com/400x300/10b981/fff?text=Abstract", price: 799, salePrice: null, type: "abstract", status: "draft", downloads: 890, rating: 4.7, images: 100 },
        { _id: "4", title: "Food Photography Bundle", thumbnail: "https://via.placeholder.com/400x300/f59e0b/fff?text=Food", price: 1299, salePrice: 899, type: "food", status: "published", downloads: 234, rating: 4.5, images: 75 },
    ];
    ...
    const handleDelete = async (id) => {
        if (!confirm("Delete this photo pack?")) return;
        try { await photoService.delete(id); toast.success("Deleted"); fetchData(); }
        ...
        <div className="p-6 lg:p-8 space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-md bg-gradient-to-br from-violet-500 to-purple-500 flex items-center justify-center">
                        <FiCamera className="text-white text-lg" />
                    </div>
                    <div>
                        <h1 className="text-lg font-semibold text-gray-900 dark:text-white">Photos</h1>
                        <p className="text-xs text-gray-500">{items.length} packs</p>
                    </div>
                </div>
                <div className="flex gap-3">
                    <button onClick={fetchData} className="btn btn-ghost p-3"><FiRefreshCw className={loading ? "animate-spin" : ""} /></button>
                    <Link href="/dashboard/admin/products/photos/create" className="btn btn-primary"><FiPlus /> Add Photo Pack</Link>
                </div>
            </div>
            ...
            <div className="card border border-gray-200 dark:border-gray-700 p-4 flex flex-col md:flex-row gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                    {["all", "landscape", "portrait", "abstract", "food"].map(f => (
            ...
            {loading ? (
                ...
                <div className="card border border-gray-200 dark:border-gray-700 p-12 text-center">
                    ...
                    <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-2">No photos found</h3>
                    <p className="text-sm text-gray-500 mb-4">Get started by adding your first photo pack</p>
                    <Link href="/dashboard/admin/products/photos/create" className="btn btn-primary inline-flex"><FiPlus /> Add Photo Pack</Link>
                </div>
            ) : (
                        ...
                        <motion.div key={item._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="card border border-gray-200 dark:border-gray-700 overflow-hidden group">
                            <div className="relative aspect-[4/3] bg-gray-100 dark:bg-gray-700 overflow-hidden">
                                ...
                                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                    <Link href={`/dashboard/admin/products/photos/${item._id}`} className="w-10 h-10 rounded-full bg-white flex items-center justify-center hover:bg-primary hover:text-white"><FiEye /></Link>
                                    <Link href={`/dashboard/admin/products/photos/create?edit=${item._id}`} className="w-10 h-10 rounded-full bg-white flex items-center justify-center hover:bg-primary hover:text-white"><FiEdit /></Link>
                            ...
                            <div className="p-4">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-xs text-gray-400">{item.images} photos</span>
                                </div>
