...
import { FiSave, FiX, FiLayout, FiArrowLeft, FiPlus, FiTag, FiDollarSign, FiLayers } from "react-icons/fi";
...
export default function CreateUIKitPage() {
    ...
    useEffect(() => {
        if (isEditMode && editId) {
            const fetchData = async () => {
                setLoading(true);
                try {
                    const response = await uiKitService.getById(editId);
    ...
    const handleSubmit = async (e) => {
        ...
        setSaving(true);
        try {
            const payload = { ...formData, price: Number(formData.price), salePrice: formData.salePrice ? Number(formData.salePrice) : null, components: Number(formData.components), screens: Number(formData.screens) };
            if (isEditMode) { await uiKitService.update(editId, payload); toast.success("Updated!"); }
            else { await uiKitService.create(payload); toast.success("Created!"); }
        ...
        <div className="p-6 lg:p-8">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/admin/products/ui-kits" className="w-10 h-10 rounded-md border border-gray-200 dark:border-gray-700 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800">
                        <FiArrowLeft />
                    </Link>
                    <div>
                        <h1 className="text-lg font-semibold text-gray-900 dark:text-white">{isEditMode ? "Edit UI Kit" : "Create UI Kit"}</h1>
                        <p className="text-xs text-gray-500">{isEditMode ? "Update UI kit details" : "Add new UI kit"}</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <Link href="/dashboard/admin/products/ui-kits" className="btn btn-ghost"><FiX size={16} /> Cancel</Link>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-6">
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><FiLayout size={16} /> Basic Information</h2>
                        ...
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="flex gap-2"><input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyPress={(e) => e.key === "Enter" && (e.preventDefault(), addTag())} className="input flex-1" /><button type="button" onClick={addTag} className="btn btn-ghost"><FiPlus /></button></div>
                                    <div className="flex flex-wrap gap-2 mt-2">{formData.tags.map((tag) => (<span key={tag} className="px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded-md text-xs flex items-center gap-1">{tag}<button type="button" onClick={() => removeTag(tag)} className="hover:text-red-500"><FiX size={12} /></button></span>))}</div>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Features</label>
                                    <div className="flex gap-2"><input type="text" value={featureInput} onChange={(e) => setFeatureInput(e.target.value)} onKeyPress={(e) => e.key === "Enter" && (e.preventDefault(), addFeature())} className="input flex-1" /><button type="button" onClick={addFeature} className="btn btn-ghost"><FiPlus /></button></div>
                                    <div className="flex flex-wrap gap-2 mt-2">{formData.features.map((f) => (<span key={f} className="px-2 py-1 bg-primary/10 text-primary rounded-md text-xs flex items-center gap-1">{f}<button type="button" onClick={() => removeFeature(f)} className="hover:text-red-500"><FiX size={12} /></button></span>))}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="space-y-6">
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            ...
                            <div className="space-y-4">
                                ...
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Platform</label><select name="platform" value={formData.platform} onChange={handleChange} className="input w-full">{PLATFORM_OPTIONS.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select></div>
                                ...
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Components Count</label><input type="number" name="components" value={formData.components} onChange={handleChange} className="input w-full" min="0" /></div>
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Screens Count</label><input type="number" name="screens" value={formData.screens} onChange={handleChange} className="input w-full" min="0" /></div>
