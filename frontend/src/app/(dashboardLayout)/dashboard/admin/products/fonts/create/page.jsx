...
import { FiSave, FiX, FiType, FiArrowLeft, FiPlus, FiTag, FiDollarSign, FiLayers } from "react-icons/fi";
import { fontService, categoryService } from "@/services/api";

const FONT_TYPES = [
    { value: "sans-serif", label: "Sans Serif" },
    { value: "serif", label: "Serif" },
    { value: "script", label: "Script / Handwritten" },
...
export default function CreateFontPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        type: "sans-serif", category: "", tags: [], thumbnail: "",
        weights: 1, formats: "OTF, TTF, WOFF",
    ...
    const handleSubmit = async (e) => {
        ...
        setSaving(true);
        try {
            const payload = { ...formData, price: Number(formData.price), salePrice: formData.salePrice ? Number(formData.salePrice) : null, weights: Number(formData.weights) };
            if (isEditMode) { await fontService.update(editId, payload); toast.success("Updated!"); }
            else { await fontService.create(payload); toast.success("Created!"); }
            router.push("/dashboard/admin/products/fonts");
        ...
        <div className="p-6 lg:p-8">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/admin/products/fonts" className="w-10 h-10 rounded-md border border-gray-200 dark:border-gray-700 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800"><FiArrowLeft /></Link>
                    <div><h1 className="text-lg font-semibold text-gray-900 dark:text-white">{isEditMode ? "Edit Font" : "Create Font"}</h1><p className="text-xs text-gray-500">{isEditMode ? "Update font details" : "Add new font"}</p></div>
                </div>
                <div className="flex items-center gap-2">
                    <Link href="/dashboard/admin/products/fonts" className="btn btn-ghost"><FiX size={16} /> Cancel</Link>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-6">
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><FiType size={16} /> Basic Information</h2>
                        ...
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            ...
                            <div className="space-y-4">
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Thumbnail/Preview URL</label><input type="url" name="thumbnail" value={formData.thumbnail} onChange={handleChange} className="input w-full" /></div>
                            </div>
                        </div>
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                    ...
                    <div className="space-y-6">
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            ...
                            <div className="space-y-4">
                                ...
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Font Type</label><select name="type" value={formData.type} onChange={handleChange} className="input w-full">{FONT_TYPES.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select></div>
                                ...
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Number of Weights</label><input type="number" name="weights" value={formData.weights} onChange={handleChange} className="input w-full" min="1" /></div>
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Formats Included</label><input type="text" name="formats" value={formData.formats} onChange={handleChange} className="input w-full" placeholder="e.g. OTF, TTF, WOFF, WOFF2" /></div>
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Glyphs Count</label><input type="text" name="glyphs" value={formData.glyphs} onChange={handleChange} className="input w-full" placeholder="e.g. 500+" /></div>
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Language Support</label><input type="text" name="languages" value={formData.languages} onChange={handleChange} className="input w-full" placeholder="e.g. Latin, Cyrillic, Bengali" /></div>
