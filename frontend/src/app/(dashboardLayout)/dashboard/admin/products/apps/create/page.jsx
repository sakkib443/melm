...
import { FiSave, FiX, FiSmartphone, FiArrowLeft, FiPlus, FiTag, FiDollarSign, FiLayers } from "react-icons/fi";
import { appTemplateService, categoryService } from "@/services/api";
...
export default function CreateAppPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        framework: "flutter", category: "", tags: [], thumbnail: "",
        price: 0, salePrice: null, status: "draft", features: [], isFeatured: false,
        platforms: [], demoUrl: "", documentation: "",
    });
    ...
    useEffect(() => {
        if (isEditMode && editId) {
            const fetchData = async () => {
                setLoading(true);
                try {
                    const response = await appTemplateService.getById(editId);
                    if (response.success && response.data) {
                        ...
                        setFormData({
                            ...
                            isFeatured: data.isFeatured || false, platforms: data.platforms || [],
                            demoUrl: data.demoUrl || "", documentation: data.documentation || "",
                        });
                    }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditMode) { await appTemplateService.update(editId, payload); toast.success("Updated!"); }
            else { await appTemplateService.create(payload); toast.success("Created!"); }
            router.push("/dashboard/admin/products/apps");
        ...
        <div className="p-6 lg:p-8">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/admin/products/apps" className="w-10 h-10 rounded-md border border-gray-200 dark:border-gray-700 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800"><FiArrowLeft /></Link>
                    <div><h1 className="text-lg font-semibold text-gray-900 dark:text-white">{isEditMode ? "Edit App Template" : "Create App Template"}</h1><p className="text-xs text-gray-500">{isEditMode ? "Update template details" : "Add new app template"}</p></div>
                </div>
                <div className="flex items-center gap-2">
                    <Link href="/dashboard/admin/products/apps" className="btn btn-ghost"><FiX size={16} /> Cancel</Link>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-6">
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><FiSmartphone size={16} /> Basic Information</h2>
                        ...
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-4">Media & Links</h2>
                            <div className="space-y-4">
                                ...
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Demo URL</label><input type="url" name="demoUrl" value={formData.demoUrl} onChange={handleChange} className="input w-full" /></div>
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Documentation URL</label><input type="url" name="documentation" value={formData.documentation} onChange={handleChange} className="input w-full" /></div>
                            </div>
                        </div>
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                    ...
                    <div className="space-y-6">
                        <div className="card border border-gray-200 dark:border-gray-700 p-5">
                            ...
                            <div className="space-y-4">
                                ...
                                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Framework</label><select name="framework" value={formData.framework} onChange={handleChange} className="input w-full">{FRAMEWORK_OPTIONS.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select></div>
