...
import { uiKitService, downloadService } from "@/services/api";
...
export default function UIKitDetailPage() {
        ...
        const fetchProductData = async () => {
            ...
            try {
                ...
                if (res?.data) {
                    ...
                    const likedItems = JSON.parse(localStorage.getItem('likedUI') || '[]');
                    ...
                    try {
                        const relatedRes = await uiKitService.getAll(`?category=${res.data.category}&limit=3`);
                ...
                toast.error(language === 'bn' ? 'UI কিট লোড করতে ব্যর্থ হয়েছে' : 'Failed to load UI kit');
            } finally {
                setLoading(false);
            }
        };
    ...
    if (!product) {
        return (
            <div className="min-h-screen bg-white dark:bg-gray-950 pt-32 text-center">
                ...
                <Link href="/ui-kits" className="text-primary font-bold">Go Back</Link>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-white dark:bg-gray-950">
            ...
            <div className="pt-28 pb-4 border-b border-gray-100 dark:border-gray-800">
                <div className="container px-6 lg:px-12 max-w-[1400px] mx-auto">
                    <div className="flex items-center gap-2 text-sm text-gray-500">
                        ...
                        <span>/</span>
                        <Link href="/ui-kits" className="hover:text-primary transition-colors">UI Kits</Link>
                        <span>/</span>
            ...
            <section className="py-8 lg:py-12">
                <div className="container px-6 lg:px-12 max-w-[1400px] mx-auto">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                        ...
                        <div className="lg:col-span-7 space-y-8">
                            ...
                            >
                                ...
                                />
                                {discount > 0 && (
                                    <div className="absolute top-8 left-8 px-6 py-2.5 bg-blue-600 text-white font-black rounded-full uppercase text-xs tracking-widest shadow-xl">
                            ...
                            {/* UI Kit Specific Stats */}
                            <div className="grid grid-cols-3 gap-4">
                                <div className="p-6 bg-gray-50 dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 text-center">
                                    <FiLayers className="w-6 h-6 mx-auto mb-2 text-blue-500" />
                                    <p className="text-[10px] font-bold text-gray-500 uppercase">Screens</p>
                                    <p className="font-black text-gray-900 dark:text-white">{product.screens || '100'}+</p>
                                </div>
                                <div className="p-6 bg-gray-50 dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 text-center">
                                    <FiGrid className="w-6 h-6 mx-auto mb-2 text-blue-500" />
                                    <p className="text-[10px] font-bold text-gray-500 uppercase">Components</p>
                                    <p className="font-black text-gray-900 dark:text-white">{product.components || '200'}+</p>
                                </div>
                                <div className="p-6 bg-gray-50 dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 text-center">
                                    <FiMoon className="w-6 h-6 mx-auto mb-2 text-blue-500" />
                                    <p className="text-[10px] font-bold text-gray-500 uppercase">Dark Mode</p>
                                    <p className="font-black text-gray-900 dark:text-white">{product.darkMode ? 'YES' : 'NO'}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-5 space-y-8">
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                                <div className="flex flex-wrap items-center gap-3 mb-6">
                                    <span className="px-4 py-1.5 bg-blue-500/10 text-blue-600 text-xs font-black uppercase tracking-widest rounded-full">{product.category}</span>
                                    <div className="h-1.5 w-1.5 rounded-full bg-gray-300 dark:bg-gray-700" />
                                    <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">Premium UI Asset</span>
                                </div>
                                ...
                                <div className="flex gap-4 mb-8">
                                    {product.software?.map((sw, i) => {
                                        ...
                                        return (
                                            <div key={i} className="flex items-center gap-3 px-6 py-4 bg-gray-100 dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700">
                                                <Icon className="text-blue-500 w-8 h-8" />
                                                <span className="font-black text-gray-900 dark:text-white uppercase text-sm">{sw}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                                ...
                                <div className="sticky bottom-0 lg:static bg-white/90 dark:bg-gray-950/90 backdrop-blur-md pt-8 lg:pt-0 z-30">
                                    <div className="flex items-center justify-between mb-8">
                                        <div className="space-y-1">
                                            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Full Design License</p>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        {hasAccess ? (
                                            <motion.button
                                                ...
                                                whileTap={{ scale: 0.98 }}
                                                className="w-full h-16 bg-green-500 text-white font-black text-xl rounded-2xl shadow-2xl flex items-center justify-center gap-3 uppercase tracking-widest"
                                            >
                                                <FiDownload /> Download Design Kit
                                            </motion.button>
                                        ) : (
                                            <>
                                                <motion.button
                                                ...
                                                >
                                                    <FiShoppingCart className="inline-block mr-2" /> Buy UI Kit
                                                </motion.button>
                    ...
                    {/* Features List */}
                    <div className="mt-20">
                        <h2 className="text-3xl font-black text-gray-900 dark:text-white uppercase font-heading mb-10">Professional Design Standards</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            {["Auto Layout Support", "Responsive Breakpoints", "Design Tokens", "Organized Layers"].map((f, i) => (
                                <div key={i} className="p-6 bg-gray-50 dark:bg-gray-910 rounded-3xl border border-gray-100 dark:border-gray-800 text-center">
                                    <div className="w-12 h-12 rounded-full bg-blue-500/10 text-blue-500 flex items-center justify-center mx-auto mb-4">
                                        <FiCheck className="w-6 h-6" />
                                    </div>
                                    <span className="text-gray-900 dark:text-white font-bold text-sm uppercase tracking-wide">{f}</span>
                                </div>
                            ))}
                        </div>
                    </div>
