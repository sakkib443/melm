// ===================================================================
// CreativeHub LMS - Course Controller
...
const getCourseBySlug = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        // Check if user has liked and enrolled
        const userId = req.user?.userId;
        const isLiked = userId && course.likedBy?.some((id: any) => id.toString() === userId);
        ...
        if (userId) {
            ...
            isEnrolled = await EnrollmentService.isEnrolled(userId, course._id.toString());
        }

        res.status(200).json({
            success: true,
            message: 'Course retrieved successfully',
            data: { ...JSON.parse(JSON.stringify(course)), isLiked: !!isLiked, isEnrolled },
        });
    } catch (error) {
        next(error);
    }
